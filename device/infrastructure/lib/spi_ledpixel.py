...
class Freenove_SPI_LedPixel(object):
    ...
    def write_ws2812_numpy8(self):
        ...
        # Convert data into a one-dimensional array
    ...
    def write_ws2812_numpy4(self):
        ...
        for ibit in range(4):
            tx[3 - ibit::4] = ((d >> (2 * ibit + 1)) & 1) * \
                0x60 + ((d >> (2 * ibit + 0)) & 1) * 0x06 + 0x88
        if self.led_init_state != 0:
            if self.bus == 0:
...
if __name__ == '__main__':
    ...
    # Use MOSI for /dev/spidev0 to drive the lights
            ...
            while True:
                for j in range(255):
                    for i in range(led.led_count):
                        ...
                        led.set_led_rgb_data(i, led.wheel(
