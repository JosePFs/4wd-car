                ...
                if (self.next - self.start) > wait_ms / 1000.0:
                    ...
                    for i in range(self.strip.get_led_count()):
                        self.strip.set_led_rgb_data(
                            i, color[self.color_wipe_index % 4])
                        self.strip.show()
    ...
    def rainbowbreathing(self, wait_ms=10):
        ...
        if self.is_support_led_function == False:
            ...
            if (self.next - self.start) > wait_ms / 1000.0:
                ...
                color2 = [int(color1[0] * brightness / 100), int(color1[1]
                                                                 * brightness / 100), int(color1[2] * brightness / 100)]
                for i in range(self.strip.get_led_count()):
    ...
    def rainbowCycle(self, wait_ms=20):
        ...
        if not self.is_support_led_function:
            ...
            if (self.next - self.start > wait_ms / 1000.0):
                self.start = self.next
                for i in range(self.strip.get_led_count()):
                    self.strip.set_led_rgb_data(i, self.wheel(
                        (int(i * 256 / self.strip.get_led_count()) + self.color_wheel_value) & 255))
                self.strip.show()
    ...
    def following(self, wait_ms=50):
        ...
        if self.is_support_led_function == False:
            ...
            if (self.next - self.start > wait_ms / 1000.0):
                ...
                self.strip.set_led_rgb_data(
                    self.color_chase_rainbow_index, self.wheel((self.color_wheel_value) & 255))
                self.strip.show()
